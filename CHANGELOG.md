...
## v1.0.0-rc（2026-08-27）

**pi 扩展合规版（沙箱开发 · 未部署）**

- 恢复扩展路线：官方子目录结构 extension/workflow-gate/{index.ts,core.ts}（含 v0.3 参考实现改造）；
- 安全强化：input/命令处理器整体异常隔离（扩展异常绝不影响 pi 输入管线）；同步工厂不阻塞启动；无网络/后台资源；只读访问为主；
- 冒烟验证：pi -ne -e <沙箱路径> -p × 1（禁自动发现+免启动网络）通过；node --test 11/11；
- 运行环境未部署（待批准）。
...
- 新工具 wf_audit：审计日志读取端（此前只有写入端）——begin/check/豁免/规则变更/reset 全轨迹可追溯；
- 任务级笔记归档：wf_begin 切换任务时旧任务摘要归档至 state.history（最多 20 个），当前任务笔记不再跨任务混显；wf_notes(history=true) 回读；
- 测试 14 → 18 例全绿（新增 wf_audit 2 例 + 归档 2 例）。
